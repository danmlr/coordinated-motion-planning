...
//Avec "obligatoire" en plus de "interdit"

public class FirstAlgorithm2 extends MotionAlgorithm {
	...
	public byte[] obligatoire;
	
	/** prioriteAbsolue[i][j] est vrai si i a une priorité absolue sur j**/
	public boolean[][] prioriteAbsolue;
	...
	public FirstAlgorithm2(Instance input) {
		this.input=input;
		...
		prioriteAbsolue = new boolean[n][n];
		xmin = input.xmin - 1; 
		xmax = input.xmax + 1;
		ymin = input.ymin - 1;
		ymax = input.ymax + 1;
	...
	public void run() {
		while (!tousArrives && solution.steps.size() <= limite) {
			if (solution.steps.size() == 90)
				IO.saveSolutionToJSON(solution, input.name + "_test.json");
			computeOneStep();
		}
	...
	public void chooseObligatoires() {
		byte j;
		for (int i = 0; i < n; i++) {
			j = obligatoire[i];
			if (j >= 0 && checkMove(i, byteToMotion(j))[0] == 1) {
				currentMoves[i] = j;
			}
			obligatoire[i] = -1;
		}
	...
	public Integer[] checkMove(int i, int[] motion) {
		...
		* Injonction a la mobilite : 2,robot donnant l'injonction
		*/
		...
		while(j >=0 && currentMoves[j]==-1) { 
			if (prioriteAbsolue[j][i] && gain(j,byteToMotion(rotation180(bmotion)))>0) {
				return new Integer[] {2,j};
			}
	...
	public int[] chooseMotion(int i) {
		...
		int axeDegagement = 0, injoncteur = 0;
		int[] motionInjoncteur = new int[2]; // Mouvement que souhaite faire l'injoncteur
		byte jInjoncteur = 0;
		for (byte j = 1; j < 5; j++) {
			...
			// MAJ de degage,axeDegagement et injoncteur
			if (options[j][0] == 2) {
				degage = true;
				injoncteur = options[j][1];
				jInjoncteur = rotation180(j);
				motionInjoncteur = byteToMotion(jInjoncteur);
				if (motionInjoncteur[0] !=0) { axeDegagement = 1; } 
				if (motionInjoncteur[1] !=0) { axeDegagement = 0; } 
			}
		}
		interdit[i]=-1;
		if (ameliorations.size() == 0 && degage) { // si aucune amélioration n'est possible, on vérifie qu'on ne doit pas dégager
			for (byte sens = 1; sens < 3; sens++) { //FIXME: choisir le meilleur sens de degagement et régler le cas où on ne peut pas dégager perpendiculairement (couloir d'obstacles)
				...
				if (options[j][0]==1) { 
					mtn = (byte) j;
					obligatoire[injoncteur]=jInjoncteur;//Si on doit degager on commence par essayer de remplacer la valeur de mtn par autre chose que 0
					prioriteAbsolue[injoncteur][i] = true;
				}
			}
			if (mtn == 0 && options[jInjoncteur][0] == 1)
				mtn = jInjoncteur;
				obligatoire[injoncteur]=jInjoncteur;
				prioriteAbsolue[injoncteur][i] = true;
		}
