...
public class MoveTargets2 extends MotionAlgorithm {
	...
	public boolean[] mouvementAutorise;
	...
	public MoveTargets2(Instance input) {
		this.input=input;
		...
		mouvementAutorise = new boolean[n];
		xmin = input.xmin - 1 - p; 
		xmax = input.xmax + 1 + p;
		ymin = input.ymin - 1 - p;
		ymax = input.ymax + 1 + p;
	...
	public void moveTarget(int i) { 
		...
		for (byte j =1 ; j < 5; j++) {
			...
			if (getMapCible(x2,y2)==-2) { 
				...
				mouvementAutorise[i] = true;
		}
		
		if (!mouvementAutorise[i])
			return;
		
		if (options.size()%4==0) {
			for (int movRef = 0; movRef < 8; movRef++) {
				if (box[i][movRef] >= 0)
					mouvementAutorise[box[i][movRef]] = true;
			}
			return;
		}
